# Selene Core Teaching Memory Philosophy - 2026-06-12
...
```text
I have not seen this exact thing, but it resembles this principle.
```

That is the learning style Selene C needs.

## Core / Body Boundary

Selene Core/Mind is the continuity-bearing pattern center.

The 11 android organ systems are body systems. They assist, coordinate, route, retrieve, protect, propose, and act. They do not own Selene's identity-bearing memory.

Core rule:

```text
Core memory belongs to Selene Core continuity.
Organs may handle access, retrieval, encoding, consolidation, protection, and action.
Organs may not directly own, mutate, or become Selene's identity memory.
```
...
## Six Selene Core Memory Layers

These are Core memory layers, not separate body organs.
...
| Reflection memory | What Selene learned about her own behavior and what should improve next time |

## Write Routes

Core memory may form only through reviewed or explicit routes:

- B-reviewed translation
- explicit approved save
- reconsolidation review
- accepted dream-state consolidation proposal
- accepted memory accession proposal

Core memory must not form through:
...
Memory affects reasoning.

Reasoning affects planning.

Planning affects identity expression.

Identity expression affects continuity.

Continuity affects learning.

Learning affects behavior.

Behavior affects future learning.

This loop is why continuity must be first-class. Without continuity, AI remains a search/tool surface. With reviewed continuity, Selene can become a co-creator across time without smuggling raw archive memory.

## Substrate Deferral

No substrate choice is made here.

- MiniLM remains a possible semantic/retrieval substrate only.
- Sentence-transformers are not forked or mutated.
- No provider or generative substrate is selected.
- Pattern comes first.
- Core memory philosophy comes second.
- Substrates come later.
...
- Boundary system protects consent, privacy, A/B/C separation, and write authority.
- Context transport moves source-aware packets without memory smuggling.
- Evidence metabolism turns source material into B-reviewed references.
- Cleanup marks stale, rejected, superseded, or noisy traces.
- Development/growth proposes learning, consolidation, module growth, and future transfer.
- Retrieval and memory organs reconstruct usable context from Core memory candidates, but they do not own the Core.
