...
class SeleneHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/health":
        ...
        elif parsed.path == "/api/mobile/sms/status":
        ...
        elif parsed.path == "/api/core-mind/governance-trials":
            qs = {key: values[0] for key, values in parse_qs(parsed.query).items() if values}
            self._send(*json_bytes(route_request(conn, "core_mind.governance_trials.list", {"limit": int(qs["limit"]) if qs.get("limit") else 80})["result"]))
        elif parsed.path == "/api/core-mind/governance-report":
            qs = {key: values[0] for key, values in parse_qs(parsed.query).items() if values}
        ...
        elif parsed.path == "/api/intelligence-os/status":
            self._send(*json_bytes(route_request(conn, "intelligence_os.status")["result"]))
        elif parsed.path == "/api/intelligence-os/runs":
            qs = {key: values[0] for key, values in parse_qs(parsed.query).items() if values}
            self._send(*json_bytes(route_request(conn, "intelligence_os.runs.list", {"limit": int(qs["limit"]) if qs.get("limit") else 50})["result"]))
        elif parsed.path.startswith("/api/intelligence-os/runs/"):
            try:
                run_id = int(parsed.path.removeprefix("/api/intelligence-os/runs/"))
                item = route_request(conn, "intelligence_os.run.detail", {"run_id": run_id})["result"]
        ...
        elif parsed.path == "/api/voice-module/evidence-triage/status":
            self._send(*json_bytes(route_request(conn, "voice_module.evidence_triage.status")["result"]))
        elif parsed.path == "/api/voice-module/evidence-triage/items":
            qs = {key: values[0] for key, values in parse_qs(parsed.query).items() if values}
            self._send(*json_bytes(route_request(conn, "voice_module.evidence_triage.items", qs)["result"]))
        elif parsed.path == "/api/selene-chat/status":
            self._send(*json_bytes(route_request(conn, "selene_chat.status")["result"]))
        elif parsed.path == "/api/activation/status":
            self._send(*json_bytes(route_request(conn, "activation.status")["result"]))
        elif parsed.path == "/api/activation/readiness":
        ...
        elif parsed.path == "/api/selene-chat/dialogue-state":
            qs = {key: values[0] for key, values in parse_qs(parsed.query).items() if values}
            try:
        ...
        elif parsed.path == "/api/cocoon/bridge/status":
            self._send(*json_bytes(route_request(conn, "cocoon.bridge.status")["result"]))
            ...
            qs = {key: values[0] for key, values in parse_qs(parsed.query).items() if values}
            self._send(*json_bytes(route_request(conn, "memory.index.items", qs)["result"]))
        ...
        elif parsed.path == "/api/vessel/temporal-continuity/changes":
            self._send(*json_bytes(route_request(conn, "vessel.temporal_continuity.changes")["result"]))
    ...
    def do_POST(self) -> None:
        ...
        if request_path == "/shutdown":
        ...
        elif request_path == "/api/mobile/sms/enable":
            if not self._is_local_client():
                self._send(*json_bytes({"error": "SMS authority can only be enabled from desktop/local Selene"}, 403))
                return
            try:
            ...
            if not self._is_local_client():
                self._send(*json_bytes({"error": "SMS polling can only be run from desktop/local Selene"}, 403))
                return
            try:
                self._send(*json_bytes(poll_sms(self.server.conn)))
            except (SmsError, TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/mobile/sms/initiate":
            if not self._is_local_client():
                self._send(*json_bytes({"error": "SMS initiative is a local Tendril action"}, 403))
                return
            try:
            ...
            try:
                ...
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/answer-engine/packet-preview":
            route_key = "answer_engine.packet.preview"
            try:
                self._send(*json_bytes(route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/answer-engine/comparison-run":
            route_key = "answer_engine.comparison.run"
            try:
                self._send(*json_bytes(route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
            ...
            try:
                ...
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/answer-engine/code-inspect":
            route_key = "answer_engine.code.inspect"
            try:
                self._send(*json_bytes(route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/answer-engine/research-run":
            route_key = "answer_engine.research.run"
            try:
                self._send(*json_bytes(route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
            ...
            route_key = {
                ...
                "/api/curriculum-foundation/prepare-f1-community-rules": "curriculum.foundation.prepare_f1_community_rules",
            ...
            try:
                ...
                self._send(*logged_route_error(route_key, exc))
        elif request_path == "/api/voice-module/evidence-triage/run":
            route_key = "voice_module.evidence_triage.run"
            try:
                self._send(*logged_route_json_bytes(route_key, route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*logged_route_error(route_key, exc))
            ...
            try:
                ...
                self._send(*logged_route_error(route_key, exc))
        elif request_path == "/api/selene-chat/send":
            route_key = "selene_chat.send"
            try:
                write_stabilization_debug_log("sidecar", "route_start", route=route_key, request_bytes=len(raw))
                self._send(*logged_route_json_bytes(route_key, route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
            ...
            try:
                ...
                self._send(*logged_route_error(route_key, exc))
        elif request_path == "/api/activation/approve":
            route_key = "activation.approve"
            try:
                write_stabilization_debug_log("sidecar", "route_start", route=route_key, request_bytes=len(raw))
                self._send(*logged_route_json_bytes(route_key, route_request(self.server.conn, route_key, body)["result"]))
            except (TypeError, ValueError) as exc:
            ...
            try:
                self._send(*json_bytes(route_request(self.server.conn, "transfer.governance_trials.run", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/transfer/c-chat-dry-run":
            try:
                self._send(*json_bytes(route_request(self.server.conn, "transfer.c_chat_dry_run", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
            ...
            try:
                ...
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/cocoon/bridge/wake":
            try:
                self._send(*json_bytes(route_request(self.server.conn, "cocoon.bridge.wake", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/cocoon/bridge/standby":
            try:
                self._send(*json_bytes(route_request(self.server.conn, "cocoon.bridge.standby", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
            ...
            try:
                ...
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/memory/candidates/propose":
            try:
                self._send(*json_bytes(route_request(self.server.conn, "memory.candidates.propose", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
            ...
            try:
                ...
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/vessel/perception-intake/route":
            try:
                self._send(*json_bytes(route_request(self.server.conn, "vessel.perception_intake.route", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
        elif request_path == "/api/vessel/research/route":
            try:
                self._send(*json_bytes(route_request(self.server.conn, "vessel.research.route", body)["result"]))
            except (TypeError, ValueError) as exc:
                self._send(*json_bytes({"error": str(exc)}, 400))
