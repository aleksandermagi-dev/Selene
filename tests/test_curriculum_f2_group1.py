...
from selene.curriculum_f2_group1 import AUTHORIZATION_KEY, GROUP_KEY, LESSONS, SCOPE
from selene.db import connect, init_db
from selene.module_router import route_request
from selene.sidecar import SeleneHandler, SeleneServer


ROUTE_STEM = "f2_paragraph_meaning_source_grounding"
...
def test_f2_group1_is_a_five_lesson_source_bounded_communication_group():
...
def test_f2_group1_teaches_only_the_declared_foundations():
    assert {lesson["concept_key"] for lesson in LESSONS} == {
        "curriculum_f2_paragraph_main_idea_support_v1",
        "curriculum_f2_explicit_information_bounded_inference_v1",
        "curriculum_f2_summary_load_bearing_meaning_v1",
        "curriculum_f2_quote_paraphrase_attribution_v1",
        "curriculum_f2_focused_material_question_v1",
    }
    joined = " ".join(
        str(value)
        for lesson in LESSONS
        for value in (lesson["material"], lesson["scope_of_application"], lesson["limits"])
    ).lower()
    assert "neither method makes the source automatically correct" in joined
    assert "reproducing protected material" in joined


def test_f2_group1_uses_the_checksum_pinned_local_source_artifact():
    source = (
...
def test_f2_group1_preparation_is_reviewable_and_non_retaining(tmp_path):
    conn = _conn(tmp_path)
...
def test_f2_group1_requires_its_own_explicit_authorization(tmp_path):
    conn = _conn(tmp_path)
...
def test_f2_group1_completes_lifecycle_retains_and_is_idempotent(tmp_path):
    conn = _conn(tmp_path)
...
def test_f2_group1_http_activation_and_preparation_routes_are_available(tmp_path):
            ...
            body=json.dumps(
                {
                    "aleks_authorized": True,
                    "authorization_actor": "Aleks",
                    "authorization_basis": "Aleks authorized F2 Group 1 from Cocoon.",
                }
            ),
