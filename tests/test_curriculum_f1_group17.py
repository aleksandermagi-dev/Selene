...
from selene.curriculum_f1_group17 import AUTHORIZATION_KEY, GROUP_KEY, LESSONS, SCOPE
...
def _authorize(conn):
    return route_request(
        ...
        f"curriculum.authorization.activate_{ROUTE_STEM}",
        {
            ...
            "authorization_basis": "Aleks authorized this bounded F1 public-academic closure bridge.",
        },
    )["result"]
...
def test_group17_is_a_four_lesson_source_bounded_f1_closure_bridge():
...
def test_group17_teaches_only_the_audited_bridges_and_defers_formal_symmetry():
    keys = {lesson["concept_key"] for lesson in LESSONS}
    assert keys == {
        "curriculum_f1_story_informational_text_purpose_v1",
        "curriculum_f1_question_roles_evidence_fit_v1",
        "curriculum_f1_needs_wants_context_criteria_v1",
        "curriculum_f1_goods_services_tools_roles_v1",
    }
    text = " ".join(
        str(value)
        for lesson in LESSONS
    ...
    assert "formal symmetry" not in text
    assert "line of symmetry" not in text
...
def test_group17_completes_lifecycle_retains_and_is_idempotent(tmp_path):
    ...
    assert status["seventeenth_group"]["retained_count"] == 4
...
def test_group17_http_activation_and_preparation_routes_are_available(tmp_path):
            ...
            body=json.dumps(
                {
                    "aleks_authorized": True,
                    "authorization_actor": "Aleks",
                    "authorization_basis": "Aleks authorized this bounded F1 closure bridge from Cocoon.",
                }
            ),
    ...
    assert preparation_payload["created_count"] == 4
