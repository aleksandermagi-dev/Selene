...
from selene.curriculum_f1_group14 import AUTHORIZATION_KEY, GROUP_KEY, LESSONS, SCOPE
...
def _authorize(conn):
    return route_request(
        conn,
        "curriculum.authorization.activate_f1_weather_sky_cycles",
        {
            "aleks_authorized": True,
            "authorization_actor": "Aleks",
            "authorization_basis": "Aleks authorized this bounded F1 public-academic weather and sky group.",
        },
    )["result"]
...
def test_group14_is_six_source_bounded_weather_and_sky_lessons():
...
def test_group14_preserves_timescale_viewpoint_prediction_and_safety_boundaries():
    by_key = {lesson["concept_key"]: lesson for lesson in LESSONS}
    weather = by_key["curriculum_f1_weather_conditions_climate_distinction_v1"]
    records = by_key["curriculum_f1_weather_measurement_records_v1"]
    seasons = by_key["curriculum_f1_seasonal_patterns_regions_hemispheres_v1"]
    day_night = by_key["curriculum_f1_earth_rotation_day_night_v1"]
    sky = by_key["curriculum_f1_sun_moon_star_observable_patterns_v1"]
    prediction = by_key["curriculum_f1_recurring_patterns_bounded_prediction_v1"]

    assert "particular place and time" in weather["material"]
    assert "long-term climate pattern" in " ".join(weather["principles"])
    assert "missing entries" in " ".join(records["principles"])
    assert "opposite seasonal timing" in seasons["material"]
    assert "false simple cause" in " ".join(seasons["limits"])
    assert "earth rotates" in day_night["material"].lower()
    assert "apparent motion" in day_night["material"].lower()
    assert "not changing its physical shape" in sky["material"]
    assert "never look directly at the sun" in " ".join(sky["principles"]).lower()
    assert "current official local warning" in prediction["participation"]
    assert "does not issue forecasts" in " ".join(prediction["limits"])


def test_group14_preparation_is_reviewable_and_non_retaining(tmp_path):
    conn = _conn(tmp_path)
    result = route_request(conn, "curriculum.foundation.prepare_f1_weather_sky_cycles", {})["result"]
...
def test_group14_requires_its_own_explicit_authorization(tmp_path):
    conn = _conn(tmp_path)
    with pytest.raises(ValueError, match="activate this bounded F1 curriculum authorization"):
        route_request(conn, "curriculum.foundation.teach_f1_weather_sky_cycles", {})
    assert conn.execute("SELECT COUNT(*) FROM selene_comprehension_concepts").fetchone()[0] == 0


def test_group14_completes_lifecycle_retains_and_is_idempotent(tmp_path):
    conn = _conn(tmp_path)
    authorization = _authorize(conn)
    result = route_request(conn, "curriculum.foundation.teach_f1_weather_sky_cycles", {})["result"]
    ...
    assert status["fourteenth_group"]["retained_count"] == 6
    ...
    repeated = route_request(conn, "curriculum.foundation.teach_f1_weather_sky_cycles", {})["result"]
    assert repeated["retained_count"] == 0
    assert repeated["already_retained_count"] == 6
    assert repeated["held_count"] == 0


def test_group14_http_activation_and_preparation_routes_are_available(tmp_path):
            ...
            body=json.dumps(
                {
                    "aleks_authorized": True,
                    "authorization_actor": "Aleks",
                    "authorization_basis": "Aleks authorized this bounded F1 weather-and-sky group from Cocoon.",
                }
            ),
