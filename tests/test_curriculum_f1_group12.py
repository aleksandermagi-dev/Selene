...
from selene.curriculum_f1_group12 import AUTHORIZATION_KEY, GROUP_KEY, LESSONS, SCOPE
...
def _authorize(conn):
    return route_request(
        conn,
        "curriculum.authorization.activate_f1_simple_machines",
        {
            "aleks_authorized": True,
            "authorization_actor": "Aleks",
            "authorization_basis": "Aleks authorized this bounded F1 public-academic simple machines group.",
        },
    )["result"]
...
def test_group12_is_six_source_bounded_simple_machine_lessons():
...
def test_group12_preserves_mechanical_tradeoff_and_safety_boundaries():
    by_key = {lesson["concept_key"]: lesson for lesson in LESSONS}
    machine = by_key["curriculum_f1_machine_task_force_tradeoff_v1"]
    ramp = by_key["curriculum_f1_inclined_plane_ramp_v1"]
    wheel = by_key["curriculum_f1_wheel_axle_gears_rotation_v1"]
    lever = by_key["curriculum_f1_lever_fulcrum_arms_v1"]
    redirects = by_key["curriculum_f1_pulley_wedge_screw_force_direction_v1"]
    compound = by_key["curriculum_f1_compound_machine_design_system_v1"]

    assert "does not create energy" in machine["material"].lower()
    assert "longer" in ramp["material"].lower() and "less force" in ramp["material"].lower()
    assert "not automatically" in " ".join(wheel["counterexamples"]).lower()
    assert "pinch" in " ".join(lever["principles"]).lower()
    assert "fixed pulley" in " ".join(redirects["counterexamples"]).lower()
    assert "no authorization" in compound["scope_of_application"].lower()


def test_group12_preparation_is_reviewable_and_non_retaining(tmp_path):
    conn = _conn(tmp_path)
    result = route_request(conn, "curriculum.foundation.prepare_f1_simple_machines", {})["result"]
...
def test_group12_requires_its_own_explicit_authorization(tmp_path):
    conn = _conn(tmp_path)
    with pytest.raises(ValueError, match="activate this bounded F1 curriculum authorization"):
        route_request(conn, "curriculum.foundation.teach_f1_simple_machines", {})
    assert conn.execute("SELECT COUNT(*) FROM selene_comprehension_concepts").fetchone()[0] == 0


def test_group12_completes_lifecycle_retains_and_is_idempotent(tmp_path):
    conn = _conn(tmp_path)
    authorization = _authorize(conn)
    result = route_request(conn, "curriculum.foundation.teach_f1_simple_machines", {})["result"]
    ...
    assert status["twelfth_group"]["retained_count"] == 6
    ...
    repeated = route_request(conn, "curriculum.foundation.teach_f1_simple_machines", {})["result"]
    assert repeated["retained_count"] == 0
    assert repeated["already_retained_count"] == 6
    assert repeated["held_count"] == 0


def test_group12_http_activation_and_preparation_routes_are_available(tmp_path):
            ...
            body=json.dumps(
                {
                    "aleks_authorized": True,
                    "authorization_actor": "Aleks",
                    "authorization_basis": "Aleks authorized this bounded F1 simple-machines group from Cocoon.",
                }
            ),
